...
def plot(a, b, c, x, l, n):
	...
	if n == 1:
		...
		plt.savefig('Fewer Bits Test Loss.png')
	else:
		plt.xlabel('Lambda')
		plt.savefig('Fewer Bits Performance.png')
	plt.close()
...
class FTRL(object):
	...
	def __init__(self, x, y):
		...
		self.w = self.w.astype('float16')
		self.z = self.z.astype('float16')
		self.pl_rate = 0.0
	...
	def save_model(self):
		np.savez_compressed('Model_fewer_bits', w=self.w, z=self.z)
...
def main():
	...
	for l in lmbda:
		...
		if a < prev_auc_loss:
			...
			plot(tr_auc_loss, tr_log_loss, tr_sq_error, r, 'Fewer Bits Test_loss, lambda= ' + str(best_l), 1)
	plot(te_auc_loss, te_log_loss, te_sq_error, lmbda, 'Fewer Bits Performance', 2)
